...
| `data/warehouse/` | &lt;0.5 GB | **Turso** (`control_plane.db`) — see `docs/TURSO.md` |
...
**Monorepo:** shared types, one API for the dashboard, benchmark history in **Turso** warehouse (`docs/TURSO.md` — implement Phase 1.5 step-by-step from official docs). `apps/dashboard` is Bun/Vite sibling (not a uv member).
...
│   ├── adapters/
│   ├── exports/
│   └── warehouse/                 # Turso SSOT for dashboard + trends (docs/TURSO.md)
│       └── control_plane.db
│
...
Stack: **`apps/api`** (FastAPI) reads/writes Turso `warehouse` + scans `data/`; **`apps/dashboard`** (Vite + React + shadcn + **Recharts** + TanStack Table/Query). Run: `uv run api` + `bun dev` on host (no compose). Jobs: Turso rows + single consumer subprocess (no Redis). Warehouse implementation: follow `docs/TURSO.md` in order.
