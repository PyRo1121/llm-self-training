...
| **1.5** | 1 | Control plane | Turso `control_plane.db` — `docs/TURSO.md` step-by-step + API + dashboard |
...
- [x] `filters.py` + `scan-raw` (regex; gitleaks per-file opt-in) — `docs/PHASE1-FILTERS.md`
...
**Warehouse:** Turso — implement by following [docs/TURSO.md](docs/TURSO.md) in order (official docs linked per step). Local file uses **pyturso** when `warehouse.driver: turso`.

- [x] Step 0–2 in `docs/TURSO.md`: CLI + schema + Python SDK (`warehouse-smoke`)
...
**Runbook:** [`docs/PHASE2-TRAIN.md`](docs/PHASE2-TRAIN.md) — Chronicals + TRL + PEFT (Context7 `/huggingface/trl` when auditing).
